...
blossom_col <- sapply( blossom_col , function(x) col.alpha( x , bloss_alpha ) )
...
for ( i in 1:tot_frames ) {
    ...
    par( mfrow=c(1,1) , mgp = c(1.25, 0.25, 0), mar = c(0.75, 2.5, 0.75, 0.75) + 0.1, 
            tck = -0.02, cex.axis = 0.8 , bg="black" )
    ...
    plot( NULL , cex=xcex , ylab="" , xlim=xlims , bty="n" , axes=FALSE , xlab="" , ylim=range(d3$doy) , bg="black" )
    l <- link( m2 )
    li <- apply(l,2,PI,0.9)
    points( dd$year , dd$doy , col=blossom_col ,  pch=8  , cex=xcex , lwd=2 )

    shade( li , dd$year , col=grau(0.7) )
...
# convert -alpha remove -background black -delay 10 -loop 0 frame*.png title_twinkle_dark.gif
