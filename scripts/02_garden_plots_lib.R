...
######
# new misclassification plot
# need garden function with diff possibilities for each ring

garden2 <- function( arc , possibilities , data , alpha.fade = 0.25 , hedge=0.1 , hedge1=0 , newplot=TRUE , plot.origin=FALSE , cex=1.5 , lwd=2 , adj.cex , adj.lwd , ring_dist , ... ) {
    ...
    draw_wedge <- function(r,hit_prior,arc2,hedge=0.1,hedge1=0,lines_to) {
        ...
        # is each path clear?
        hit <- hit_prior * ifelse( possibilities[[r]]==data[r] , 1 , 0 )
        ...
        # draw wedge
        hedge_use <- ifelse( r==1 , hedge1 , hedge )
        poss.cols <- ifelse( possibilities[[r]]==1 , rangi2 , "white" )
        pts <- wedge( ring_dist[r] , arc2[1] , arc2[2] , poss.cols , hedge=hedge_use , alpha=alpha , cex=cex*adj.cex[r] , lwd=lwd*adj.lwd[r] )
        

        
        if ( N > r ) {
            n_poss <- length(possibilities[[r+1]])
            # draw next layer
            span <- abs( arc2[1] - arc2[2] ) / n_poss
            for ( j in 1:n_poss ) {
                # for each possibility, draw the next wedge
...
garden2(
    arc = arc,
    possibilities = list( c(0,1,1,1) , c(0,0,1) ) ,
    data = dat,
    hedge = 0.05,
    ring_dist=ring_dist,
    alpha.fade=1
...
# true states
poss.cols <- c( rep(rangi2,3) , "#FFFFFF" )
pts <- wedge( ring_dist[1] , arc[1] , arc[2] , poss.cols , hedge=TRUE , alpha=rep(1,4) , cex=1.5*1 , lwd=2*1 )

for ( k in 1:4 ) {
    line.short( c(0,pts[k,1]) , c(0,pts[k,2]) , lwd=2 , short=0.04 , col=1 )
}

# observations
poss.cols <- c( rep("#FFFFFF",2) , rep(rangi2,1) )
span <- abs( arc[1] - arc[2] ) / 4
for ( j in 1:4 ) {

    if ( j > 1 ) poss.cols <- c( rep(rangi2,2) , rep("#FFFFFF",1) )

    new_arc <- c( arc[1]+span*(j-1) , arc[1]+span*j )
    pts2 <- wedge( ring_dist[2] , new_arc[1] , new_arc[2] , poss.cols , hedge=TRUE , alpha=rep(1,3) , cex=1.5*1 , lwd=2*1 )

    for ( k in 1:3 ) {
        jj <- 5-j
        line.short( c(pts[jj,1],pts2[k,1]) , c(pts[jj,2],pts2[k,2]) , lwd=2 , short=0.04 , col=1 )
    }
}#j
